...
if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID) {
  console.error(
    "❌ Не найдены переменные AIRTABLE_API_KEY / AIRTABLE_BASE_ID. Смотрите .env.example"
  );
  process.exit(1);
}

/* ----------  Основной клиент Airtable  ---------- */
...
export async function getMessagesForDay(day) {
  // в таблице Messages есть поле `Day` (Number) и `Message` (Long text)
  const formula = `{Day} = ${day}`;
  const { data } = await api.get("/Messages", {
    params: { filterByFormula: formula }
  });
  // возвращаем массив объектов вида { Message: "...", Day: N }
  ...
  try {
    await api.post("/Log", {
      fields: {
        ...
        MessageText: text            // поле MessageText – optional
      }
    });
  } catch (e) {
    console.error("⚠️ Ошибка записи лога в Airtable:", e.message);
  }
}

/* ---------- Helper: получение всех пользователей (для cron) ---------- */
